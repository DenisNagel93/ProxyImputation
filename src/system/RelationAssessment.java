...
public class RelationAssessment {
    ...
    public RelationScore checkRelation(InstanceMatch iA, InstanceMatch iB, NarrativeRelation nr) {
        ...
        if (eA.getD().getJoinCandidates(eB.getD()).size() > 0) {
            HashSet<Attribute> constraintA = eA.getAssociatedAttributes();
            constraintA = refineJoinConstraints(iA,constraintA);
            HashSet<Attribute> constraintB = eB.getAssociatedAttributes();
            constraintB = refineJoinConstraints(iB,constraintB);
            Join j = new Join(iA, iB, eA.getD().getJoinCandidates(eB.getD()),constraintA,constraintB);
    ...
    public HashSet<Attribute> refineJoinConstraints(InstanceMatch i, HashSet<Attribute> constraint) {
        HashSet<Attribute> newConstraint = new HashSet<>();
        for (Attribute a : constraint) {
